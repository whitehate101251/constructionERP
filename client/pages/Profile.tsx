...
export default function Profile() {
  ...
  const onSave = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await fetch(`https://erp-backend-lm5c.onrender.com/api/admin/users/${user?.id}`, {
        method: "PUT",
        headers: {
  ...
  const onChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`https://erp-backend-lm5c.onrender.com/api/auth/change-password`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("auth_token")}`,
