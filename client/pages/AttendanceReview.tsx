...
export default function AttendanceReview() {
  ...
  const fetchPendingRecords = async () => {
    try {
      const response = await fetch(`https://erp-backend-lm5c.onrender.com/api/attendance/pending-review`, {
        headers: {
  ...
  const submitReview = async (action: 'approve' | 'reject') => {
    ...
    try {
      ...
      const response = await fetch(`https://erp-backend-lm5c.onrender.com/api/attendance/review/${selectedRecord.id}`, {
        method: "POST",
        headers: {
